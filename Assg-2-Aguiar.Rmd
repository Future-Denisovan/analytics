...
author: "Caleb Aguiar & Annie Beyer"
date: "January 2021"
...
for (a in seq(0.01, 0.9, by=0.01)) {
  ...
  newrow2<-c(a,myprofit[2,1])
  df2<-rbind(df2,newrow2)

  if (myprofit[2,1]>1){
    newrow<-c(a,myprofit[2,1])
    df<-rbind(df,newrow)
  }
...
colnames(df)<-c("Cost","Profit_Change")
df


colnames(df2)<-c("Cost","Profit_Change")
df2
...
ggplot(data=df)+geom_point(mapping = aes(x = Cost, y = Profit_Change))
ggplot(data=df2)+geom_point(mapping = aes(x = Cost, y = Profit_Change))
