...
```{r}
#Don't run this unless you want all the dummies printed
#summary(lm128_B)
#summary(lm64_B)
```
...
Time trend is extremely important, once we accounted for seasonal trends, it improves the previous model immensely. This makes sense, since the purchase of detergent is likely to be driven by weather(depends on the location, but think about people spending alot of time outside vs inside would change the rate at which the product is used). It is important not to try to build too many assumptions into this. Tide128 is a product that may take a few weeks to consume, and so if there is a seasonal effect driving its consumption there will be a lag effect. The big takeaway is that incorporating the week that the detergent was purchased into our model we get a much more accurate model.

See the table below for a comparison of the multiple R squared values. Once we implemented dummy variables for time, our model was able to capture between roughly 40-60% more of the variation.
...
I like graphs, so see below for a graph of the coefficients in relation to each week. There is clearly some cyclical seasonal pattern for Tide64 and Tide128. We're primarily interested in the width of the confidence interval here for each coefficient. After about week 187, both models become less confident in its predictions based on the week. 
...
lm128_C = lm(log(svelocity128) ~ log(p_tide128)+log(p_tide64) + factor(week), data = detergent_DF_2)
lm64_C = lm(log(svelocity64) ~ log(p_tide128)+log(p_tide64) + factor(week), data = detergent_DF_2)
...
```{r}
lm128_D<-lm(log(svelocity128) ~ log(p_tide128)+log(p_tide64) + factor(week) + factor(store), data = detergent_DF_2)
lm64_D <- lm(log(svelocity64) ~ log(p_tide128)+log(p_tide64)+ factor(week)+ factor(store), data = detergent_DF_2)
